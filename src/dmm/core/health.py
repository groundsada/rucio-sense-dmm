...
The frontend runs in a `multiprocessing.Process`, so every daemon heartbeat the
daemons keep in memory — including all of their prometheus metrics — is
invisible to the process that answers `/health`. The two share only the
database, and the database is one of the things health has to be able to report
on, so it cannot be the channel. A directory of small files can be written by
the parent and read by the child without either knowing the other exists.
"""
import json
import logging
import os
